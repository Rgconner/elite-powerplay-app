"""
Seed sample reference and capture images so the WSL mock mode has something
to work with right out of the box.

This is also the implementation behind `python -m visinsp.services.cli seed`.

Creates:
  data/references/reference_good.png  (a synthetic "good" part)
  data/references/reference_defect.png (a synthetic "defect" part)
...
def seed(paths: Paths, count: int = 5) -> None:
    ...
    print(f"  wrote {ref_good_path}")
    print(f"  wrote {ref_bad_path}")
