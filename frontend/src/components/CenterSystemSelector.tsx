...
export default function CenterSystemSelector({ value, onChange }: Props) {
    ...
    <div ref={wrapRef} style={{ position: "relative", display: "inline-block", minWidth: 240 }}>
      <div style={{ display: "flex", alignItems: "center", border: "1px solid #e5e7eb", borderRadius: 6, background: "#fff", padding: "0 8px" }}>
        <input
          ...
          placeholder="Center system (optional)..."
      ...
      {open && results.length > 0 && (
        <ul style={{ position: "absolute", top: "calc(100% + 2px)", left: 0, right: 0, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 6, listStyle: "none", margin: 0, padding: "4px 0", zIndex: 999, maxHeight: 200, overflowY: "auto", boxShadow: "0 4px 12px rgba(0,0,0,.08)" }}>
          {results.map((r) => (
            <li
              key={r.system_id64}
